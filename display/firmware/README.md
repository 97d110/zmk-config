...
marks that the current typing check period saw input and starts a delayed work
cycle if one is not already running. The delayed work logs the complete display
state once per period, keeps semantic activity at `typing` while keypresses are
observed, and returns activity to `idle` after a quiet period.
